...
const marca = 'Originales Outlet';

const goBack = () => {
    router.push('/')
}

// Importamos las variables y funciones necesarias
const {
    perfumes,
    loading,
    cart,
    ...
    <div class="bg-stone-900 font-sans text-stone-200 min-h-screen">
        ...
        <section class="relative py-16 px-6 text-center bg-gradient-to-b from-stone-950 to-stone-900 border-b border-stone-800">
            <div class="max-w-7xl mx-auto relative">
                ...
                <p class="text-stone-400 font-light uppercase tracking-widest text-sm">Oportunidades Únicas</p>
            </div>
        </section>

        <!-- Lista de Perfumes -->
        <section class="py-16 px-6 max-w-7xl mx-auto relative">
            <!-- Estado de carga -->
            <div v-if="loading" class="text-center text-stone-400 text-lg">Cargando perfumes...</div>
            
            <!-- Estado sin perfumes -->
            <div v-else-if="perfumes.length === 0" class="text-center text-stone-400 text-lg">No hay perfumes disponibles en esta categoría por el momento.</div>
        ...
        <div v-if="showAddModal" class="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" @click.self="showAddModal = false">
            <div class="bg-stone-900 border border-green-600/50 p-6 max-w-md w-full rounded-sm shadow-2xl relative max-h-[90vh] overflow-y-auto">
                ...
                <form @submit.prevent="createPerfume" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-stone-400 text-xs uppercase tracking-wider mb-1">Nombre</label>
                        <input v-model="newPerfume.nombre" class="w-full bg-stone-800 border border-stone-700 p-2 text-stone-200 focus:border-green-600 outline-none" required />
                    </div>
                    <div>
                        <label class="block text-stone-400 text-xs uppercase tracking-wider mb-1">Precio</label>
                        <input v-model="newPerfume.precio" type="number" class="w-full bg-stone-800 border border-stone-700 p-2 text-stone-200 focus:border-green-600 outline-none" required />
                    </div>
                    <div>
                        <label class="block text-stone-400 text-xs uppercase tracking-wider mb-1">Marca</label>
                        <input v-model="newPerfume.marca" disabled class="w-full bg-stone-800 border border-stone-700 p-2 text-stone-200 focus:border-green-600 outline-none" required />
                    </div>
                    <div>
                        <label class="block text-stone-400 text-xs uppercase tracking-wider mb-1">Categoría</label>
                        <select v-model="newPerfume.categoria" class="w-full bg-stone-800 border border-stone-700 p-2 text-stone-200 focus:border-green-600 outline-none">
                            <option value="Masculino">Masculino</option>
                            <option value="Femenino">Femenino</option>
                            <option value="Unisex">Unisex</option>
                        </select>
            ...
            <div class="bg-stone-900 border-l border-stone-700 w-full sm:w-96 h-full flex flex-col shadow-2xl transform transition-transform">
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4">
                    <div v-for="(item, index) in cart" :key="index" class="flex gap-4 items-center bg-stone-800 p-3 rounded-sm border border-stone-700">
                        <img :src="getImageUrl(item.imagen)" loading="lazy" decoding="async" class="w-16 h-16 object-cover rounded-sm" />
                        <div class="flex-1"><h4 class="text-stone-200 font-bold text-sm">{{ item.nombre }}</h4><p class="text-amber-600 font-bold">${{ item.precio }}</p></div>
